...
datanorm.drop(['snow'], axis=1, inplace=True)
datanorm['good_weather'] = datanorm[['precip', 'snowdepth', 'windspeed', 'cloudcover']].mean(axis=1).apply(lambda x: 1 if x < 0.5 else 0)
datanorm['is_day'] = datanorm['hour_of_day'].apply(lambda x: 1 if 6 <= x < 20 else 0)
